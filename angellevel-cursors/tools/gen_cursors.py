#!/usr/bin/env python3
"""
Generate the AngelLevel IRIX Xcursor theme and its preview contact sheet.
...
Outputs (relative to the repo):
    angellevel-cursors/AngelLevel-IRIX/{index.theme,cursor.theme,cursors/*}
    angellevel-cursors/previews/cursors.png
"""
import os
import struct
import sys
...
THEME = os.path.join(ROOT, 'AngelLevel-IRIX')
...
def build_theme():
    ...
    with open(os.path.join(THEME, 'index.theme'), 'w') as f:
        f.write('[Icon Theme]\n'
                'Name=AngelLevel IRIX\n'
                'Comment=SGI IRIX-idiom pointers (TenshiNET / AngelLevel): bold '
                'black\n shapes with a white outline, engineering crosshair, '
                'Motif wristwatch.\n'
                'Inherits=Adwaita\n')
    with open(os.path.join(THEME, 'cursor.theme'), 'w') as f:
        f.write('[Icon Theme]\nName=AngelLevel IRIX\nInherits=AngelLevel-IRIX\n')
    ...
    for k in range(2):
        ...
        if k == 0:
            ...
            d.rectangle([qx + bs // 2 - m, by + bs // 2 - m, qx + bs // 2 + m, by + bs // 2 + m],
                        outline=INK, width=px(1))
    d.text(((tx0 + tx1) // 2, (ty0 + ty1) // 2), 'AngelLevel  IRIX  —  Pointer Set',
