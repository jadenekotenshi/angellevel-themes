# AngelLevel-darkmode for KDE Plasma
...
|-------|------|----------------|
| Color scheme | `color-schemes/AngelLevel-darkmode.colors` | App colors — NeXT grays, white views, dark selection |
| Window decoration | `aurorae/AngelLevel-darkmode/` | Aurorae title bars & borders with beveled square buttons |
| Konsole scheme | `konsole/AngelLevel-darkmode.colorscheme` | Terminal palette (black on white, muted ANSI) |
| Icon theme | `icons/AngelLevel-darkmode/` | OPENSTEP-inspired **colour** icons with the chiselled bevel; inherits Breeze for the rest |
| SDDM login theme | `sddm/AngelLevel-darkmode/` (Qt5) · `sddm/qt6/AngelLevel-darkmode/` (Qt6) | OPENSTEP-style QML greeter — chiselled panel, indigo title bar, colour power buttons |
| Plasma Style | `plasma/desktoptheme/AngelLevel-darkmode/` | NeXT chiselled FrameSvg widgets (panels, plasmoids, buttons, fields, tooltips) |
| Global Theme | `plasma/look-and-feel/org.angellevel.darkmode.desktop/` | Look-and-Feel that applies the whole set + boot splash + logout screen |
| Kvantum theme | `kvantum/AngelLevel-darkmode/` | NeXT app-widget style (SVG) for the Kvantum QStyle engine |
| Qt style sheet | `qt-style/AngelLevel-darkmode.qss` | Lightweight NeXT-beveled widget QSS for any Qt app |
| Qt QStyle plugin | `qstyle/` | Native C++ widget style — the full NeXT look (twin-arrow scrollbars, metallic bars) |
| Plymouth splash | `plymouth/angellevel/` | OPENSTEP boot splash — chiselled cube + metallic-indigo progress |
...
SVGs under the matching `icons/AngelLevel-darkmode/<context>/scalable/` folder to grow
coverage.
...
cp color-schemes/AngelLevel-darkmode.colors      ~/.local/share/color-schemes/
cp -r aurorae/AngelLevel-darkmode                 ~/.local/share/aurorae/themes/
cp konsole/AngelLevel-darkmode.colorscheme        ~/.local/share/konsole/
cp -R icons/AngelLevel-darkmode                   ~/.local/share/icons/
```

## Apply

- **Colors:** System Settings → Colors → *AngelLevel-darkmode*
  (or `plasma-apply-colorscheme AngelLevel-darkmode`)
- **Window decoration:** System Settings → Window Decorations → *AngelLevel-darkmode*
- **Konsole:** Konsole → Settings → Edit Current Profile → Appearance → *AngelLevel-darkmode*
- **Icons:** System Settings → Icons → *AngelLevel-darkmode*
...
The login greeter lives under `sddm/AngelLevel-darkmode/` (a self-contained Qt Quick
...
OPENSTEP-blue gradient. See `sddm/AngelLevel-darkmode/preview.png`.
...
Then activate it in `/etc/sddm.conf.d/angellevel.conf`:

```ini
[Theme]
Current=AngelLevel-darkmode
```

Preview it without logging out:

```bash
sddm-greeter --test-mode --theme /usr/share/sddm/themes/AngelLevel-darkmode
```

Background and panel/title colours are configurable in
`sddm/AngelLevel-darkmode/theme.conf` (set `background=` to a wallpaper path, or
...
Qt5 one (`sddm/AngelLevel-darkmode/`) and a Qt6 one (`sddm/qt6/AngelLevel-darkmode/`, with
...
`plasma/look-and-feel/org.angellevel.darkmode.desktop/` is a **Global Theme**. Its
`contents/defaults` wires the whole set together — AngelLevel-darkmode colour scheme,
icon theme, Plasma Style, and Aurorae decoration — so one click applies
everything:

```bash
lookandfeeltool -a org.angellevel.darkmode.desktop     # apply
...
lock screen automatically picks up the AngelLevel-darkmode **colour scheme**, so it's
...
export QT_STYLE_OVERRIDE=AngelLevel-darkmode       # or System Settings -> Application Style
```
...
```bash
kvantum/install-kvantum.sh          # copies to ~/.config/Kvantum/AngelLevel-darkmode
kvantummanager --set AngelLevel-darkmode
# then set the application style to "Kvantum" (System Settings / qt5ct / qt6ct)
```

`kvantum/AngelLevel-darkmode/AngelLevel-darkmode.svg` (488 elements) is generated by
`tools/gen_kvantum.py`; `AngelLevel-darkmode.kvconfig` maps each widget to it.

**Qt Style Sheet** (`qt-style/AngelLevel-darkmode.qss`) — a lighter, engine-free
alternative that works in any Qt app: point qt5ct/qt6ct → *Appearance →
Style Sheets* at it, or launch an app with `-stylesheet AngelLevel-darkmode.qss`.
...
`plymouth/angellevel/` is a script-module Plymouth theme: the muted-blue
...
sudo plymouth-set-default-theme -R angellevel # set default (rebuilds initramfs)
```
...
- `AngelLevel-darkmoderc` — geometry: border thickness, title height, button size.
...
- **Title-bar height / button alignment** — tune `TitleHeight`,
  `ButtonHeight`, and `ButtonMarginTop` in `AngelLevel-darkmoderc`.
...
- **Border thickness** — `BorderLeft/Right/Bottom` in `AngelLevel-darkmoderc`.
