...
|-------|------|----------------|
| Color scheme | `color-schemes/NeXTSTEP.colors` | App colors — NeXT grays, white views, dark selection |
| Window decoration | `aurorae/NeXTSTEP/` | Aurorae title bars & borders with beveled square buttons |
| Konsole scheme | `konsole/NeXTSTEP.colorscheme` | Terminal palette (black on white, muted ANSI) |
...
cp color-schemes/NeXTSTEP.colors      ~/.local/share/color-schemes/
cp -r aurorae/NeXTSTEP                 ~/.local/share/aurorae/themes/
cp konsole/NeXTSTEP.colorscheme        ~/.local/share/konsole/
```

## Apply

- **Colors:** System Settings → Colors → *NeXTSTEP*
  (or `plasma-apply-colorscheme NeXTSTEP`)
- **Window decoration:** System Settings → Window Decorations → *NeXTSTEP*
- **Konsole:** Konsole → Settings → Edit Current Profile → Appearance → *NeXTSTEP*
...
- `NeXTSTEPrc` — geometry: border thickness, title height, button size.
...
- **Title-bar height / button alignment** — tune `TitleHeight`,
  `ButtonHeight`, and `ButtonMarginTop` in `NeXTSTEPrc`.
...
- **Border thickness** — `BorderLeft/Right/Bottom` in `NeXTSTEPrc`.
