...
|-------|------|----------------|
| Color scheme | `color-schemes/AngelLevel.colors` | App colors — NeXT grays, white views, dark selection |
| Window decoration | `aurorae/AngelLevel/` | Aurorae title bars & borders with beveled square buttons |
| Konsole scheme | `konsole/AngelLevel.colorscheme` | Terminal palette (black on white, muted ANSI) |
| Icon theme | `icons/AngelLevel/` | OPENSTEP-inspired **colour** icons with the chiselled bevel; inherits Breeze for the rest |
| SDDM login theme | `sddm/AngelLevel/` (Qt5) · `sddm/qt6/AngelLevel/` (Qt6) | OPENSTEP-style QML greeter — chiselled panel, indigo title bar, colour power buttons |
| Plasma Style | `plasma/desktoptheme/AngelLevel/` | NeXT chiselled FrameSvg widgets (panels, plasmoids, buttons, fields, tooltips) |
| Global Theme | `plasma/look-and-feel/org.angellevel.desktop/` | Look-and-Feel that applies the whole set + boot splash + logout screen |
| Kvantum theme | `kvantum/AngelLevel/` | NeXT app-widget style (SVG) for the Kvantum QStyle engine |
| Qt style sheet | `qt-style/AngelLevel.qss` | Lightweight NeXT-beveled widget QSS for any Qt app |
...
SVGs under the matching `icons/AngelLevel/<context>/scalable/` folder to grow
coverage.
...
cp color-schemes/AngelLevel.colors      ~/.local/share/color-schemes/
cp -r aurorae/AngelLevel                 ~/.local/share/aurorae/themes/
cp konsole/AngelLevel.colorscheme        ~/.local/share/konsole/
cp -R icons/AngelLevel                   ~/.local/share/icons/
```

## Apply

- **Colors:** System Settings → Colors → *AngelLevel*
  (or `plasma-apply-colorscheme AngelLevel`)
- **Window decoration:** System Settings → Window Decorations → *AngelLevel*
- **Konsole:** Konsole → Settings → Edit Current Profile → Appearance → *AngelLevel*
- **Icons:** System Settings → Icons → *AngelLevel*
...
The login greeter lives under `sddm/AngelLevel/` (a self-contained Qt Quick
...
OPENSTEP-blue gradient. See `sddm/AngelLevel/preview.png`.
...
```bash
sddm-greeter --test-mode --theme /usr/share/sddm/themes/AngelLevel
```

Background and panel/title colours are configurable in
`sddm/AngelLevel/theme.conf` (set `background=` to a wallpaper path, or
...
Qt5 one (`sddm/AngelLevel/`) and a Qt6 one (`sddm/qt6/AngelLevel/`, with
...
`plasma/look-and-feel/org.angellevel.desktop/` is a **Global Theme**. Its
`contents/defaults` wires the whole set together — AngelLevel colour scheme,
icon theme, Plasma Style, and Aurorae decoration — so one click applies
everything:

```bash
lookandfeeltool -a org.angellevel.desktop     # apply
...
lock screen automatically picks up the AngelLevel **colour scheme**, so it's
...
export QT_STYLE_OVERRIDE=AngelLevel       # or System Settings -> Application Style
```
...
```bash
kvantum/install-kvantum.sh          # copies to ~/.config/Kvantum/AngelLevel
kvantummanager --set AngelLevel
# then set the application style to "Kvantum" (System Settings / qt5ct / qt6ct)
```

`kvantum/AngelLevel/AngelLevel.svg` (488 elements) is generated by
`tools/gen_kvantum.py`; `AngelLevel.kvconfig` maps each widget to it.

**Qt Style Sheet** (`qt-style/AngelLevel.qss`) — a lighter, engine-free
alternative that works in any Qt app: point qt5ct/qt6ct → *Appearance →
Style Sheets* at it, or launch an app with `-stylesheet AngelLevel.qss`.
...
- `AngelLevelrc` — geometry: border thickness, title height, button size.
...
- **Title-bar height / button alignment** — tune `TitleHeight`,
  `ButtonHeight`, and `ButtonMarginTop` in `AngelLevelrc`.
...
- **Border thickness** — `BorderLeft/Right/Bottom` in `AngelLevelrc`.
