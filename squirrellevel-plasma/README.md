...
kvantummanager --set SquirrelLevel
# then set the application style to "Kvantum" (System Settings / qt5ct / qt6ct)
```

`kvantum/SquirrelLevel/SquirrelLevel.svg` (488 elements) is generated by
`tools/gen_kvantum.py`; `SquirrelLevel.kvconfig` maps each widget to it.

**Qt Style Sheet** (`qt-style/SquirrelLevel.qss`) — a lighter, engine-free
