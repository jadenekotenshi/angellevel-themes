# AngelLevel for rEFInd

A [rEFInd](https://www.rodsbooks.com/refind/) boot-manager theme visually
coherent with the AngelLevel Plasma theme — the OPENSTEP muted-blue gradient
...
| Folder | Look |
|--------|------|
| `AngelLevel/`      | Light — OPENSTEP-blue banner, gunmetal-silver selection tile, indigo frame |
| `AngelLevel-dark/` | Dark — near-black banner, dark gunmetal selection tile, bright-indigo frame |
...
- `icons/` — a coherent icon set in the AngelLevel palette:
...
sudo cp -r AngelLevel "$REFIND/themes/"
# add this line to the end of $REFIND/refind.conf:
#   include themes/AngelLevel/theme.conf
```

(For the dark variant, copy `AngelLevel-dark` and include
`themes/AngelLevel-dark/theme.conf` instead.) Only one theme should be
`include`d at a time.

## EFI boot splash (seamless fade)

`efi-splash/AngelLevel.png` and `efi-splash/AngelLevel-dark.png` (1920×1080
...
Assets are rendered from the AngelLevel SVGs (transparency recovered via a
