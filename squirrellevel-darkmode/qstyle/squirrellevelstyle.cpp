...
static const QColor kRadio (0x2a, 0x24, 0x78);
...
static void drawArrow(QPainter *p, const QRect &r, Qt::ArrowType dir,
                      const QColor &col = kDark)
{
    const qreal h = qMax(3, qMin(r.width(), r.height()) / 4);
...
static void applyPalette(QPalette &pal)
{
    ...
    pal.setColor(QPalette::Disabled, QPalette::WindowText, QColor(0x6a, 0x6f, 0x78));
    pal.setColor(QPalette::Disabled, QPalette::Text,       QColor(0x6a, 0x6f, 0x78));
    pal.setColor(QPalette::Disabled, QPalette::ButtonText, QColor(0x6a, 0x6f, 0x78));
}

// --- style ------------------------------------------------------------------

SquirrelLevel-darkmodeStyle::SquirrelLevel-darkmodeStyle() : QProxyStyle(QStringLiteral("Fusion")) {}

void SquirrelLevel-darkmodeStyle::polish(QPalette &palette) { applyPalette(palette); }

QPalette SquirrelLevel-darkmodeStyle::standardPalette() const
{
...
int SquirrelLevel-darkmodeStyle::pixelMetric(PixelMetric m, const QStyleOption *o, const QWidget *w) const
{
    switch (m) {
...
void SquirrelLevel-darkmodeStyle::drawPrimitive(PrimitiveElement pe, const QStyleOption *opt,
                                       QPainter *p, const QWidget *w) const
{
    ...
    case PE_IndicatorCheckBox: {
        paintBevel(p, opt->rect, QColor(0x20, 0x22, 0x26), true,
        ...
        if (opt->state & State_On) {
            ...
            p->save();
            p->setRenderHint(QPainter::Antialiasing, true);
            QPen pen(kGreen, 2.2); pen.setCapStyle(Qt::RoundCap); pen.setJoinStyle(Qt::RoundJoin);
...
void SquirrelLevel-darkmodeStyle::drawControl(ControlElement ce, const QStyleOption *opt,
                                     QPainter *p, const QWidget *w) const
{
    switch (ce) {
...
QRect SquirrelLevel-darkmodeStyle::subControlRect(ComplexControl cc, const QStyleOptionComplex *opt,
...
void SquirrelLevel-darkmodeStyle::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
                                            QPainter *p, const QWidget *w) const
{
    if (cc == CC_ScrollBar) {
