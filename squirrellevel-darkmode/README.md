# SquirrelLevel-darkmode for KDE Plasma
...
**Wallpaper / Plymouth handoff.** `wallpaper/TenshiNET-<WxH>.png` (1280×720,1920×1080, 2560×1440, 3840×2160) is the desktop background that *matches the Plymouth boot splash* — same gradient and the TenshiNET angel in the exact position Plymouth places it — so the machine fades seamlessly from boot into the desktop. The Global Theme ships the 1080p one (`contents/wallpaper.png`, wired via `defaults`); for a pixel-exact handoff pick the file matching your native resolution and set it (desktop **and** SDDM background) with fill mode *Centered* so it is not rescaled.
...
|-------|------|----------------|
| Color scheme | `color-schemes/SquirrelLevel-darkmode.colors` | App colors — NeXT grays, white views, dark selection |
| Window decoration | `aurorae/SquirrelLevel-darkmode/` | Aurorae title bars & borders with beveled square buttons |
| Konsole scheme | `konsole/SquirrelLevel-darkmode.colorscheme` | Terminal palette (black on white, muted ANSI) |
| Icon theme | `icons/SquirrelLevel-darkmode/` | OPENSTEP-inspired **colour** icons with the chiselled bevel; inherits Breeze for the rest |
| SDDM login theme | `sddm/SquirrelLevel-darkmode/` (Qt5) · `sddm/qt6/SquirrelLevel-darkmode/` (Qt6) | OPENSTEP-style QML greeter — chiselled panel, indigo title bar, colour power buttons |
| Plasma Style | `plasma/desktoptheme/SquirrelLevel-darkmode/` | NeXT chiselled FrameSvg widgets (panels, plasmoids, buttons, fields, tooltips) |
| Global Theme | `plasma/look-and-feel/org.squirrellevel.darkmode.desktop/` | Look-and-Feel that applies the whole set + boot splash + logout screen |
| Kvantum theme | `kvantum/SquirrelLevel-darkmode/` | NeXT app-widget style (SVG) for the Kvantum QStyle engine |
| Qt style sheet | `qt-style/SquirrelLevel-darkmode.qss` | Lightweight NeXT-beveled widget QSS for any Qt app |
...
SVGs under the matching `icons/SquirrelLevel-darkmode/<context>/scalable/` folder to grow
coverage.
...
cp color-schemes/SquirrelLevel-darkmode.colors      ~/.local/share/color-schemes/
cp -r aurorae/SquirrelLevel-darkmode                 ~/.local/share/aurorae/themes/
cp konsole/SquirrelLevel-darkmode.colorscheme        ~/.local/share/konsole/
cp -R icons/SquirrelLevel-darkmode                   ~/.local/share/icons/
```

## Apply

- **Colors:** System Settings → Colors → *SquirrelLevel-darkmode*
  (or `plasma-apply-colorscheme SquirrelLevel-darkmode`)
- **Window decoration:** System Settings → Window Decorations → *SquirrelLevel-darkmode*
- **Konsole:** Konsole → Settings → Edit Current Profile → Appearance → *SquirrelLevel-darkmode*
- **Icons:** System Settings → Icons → *SquirrelLevel-darkmode*
...
The login greeter lives under `sddm/SquirrelLevel-darkmode/` (a self-contained Qt Quick
...
OPENSTEP-blue gradient. See `sddm/SquirrelLevel-darkmode/preview.png`.
...
```ini
[Theme]
Current=SquirrelLevel-darkmode
```

Preview it without logging out:

```bash
sddm-greeter --test-mode --theme /usr/share/sddm/themes/SquirrelLevel-darkmode
```

Background and panel/title colours are configurable in
`sddm/SquirrelLevel-darkmode/theme.conf` (set `background=` to a wallpaper path, or
...
Qt5 one (`sddm/SquirrelLevel-darkmode/`) and a Qt6 one (`sddm/qt6/SquirrelLevel-darkmode/`, with
...
`plasma/look-and-feel/org.squirrellevel.darkmode.desktop/` is a **Global Theme**. Its
`contents/defaults` wires the whole set together — SquirrelLevel-darkmode colour scheme,
icon theme, Plasma Style, and Aurorae decoration — so one click applies
everything:

```bash
lookandfeeltool -a org.squirrellevel.darkmode.desktop     # apply
...
lock screen automatically picks up the SquirrelLevel-darkmode **colour scheme**, so it's
...
export QT_STYLE_OVERRIDE=SquirrelLevel-darkmode       # or System Settings -> Application Style
```
...
```bash
kvantum/install-kvantum.sh          # copies to ~/.config/Kvantum/SquirrelLevel-darkmode
kvantummanager --set SquirrelLevel-darkmode
# then set the application style to "Kvantum" (System Settings / qt5ct / qt6ct)
```

`kvantum/SquirrelLevel-darkmode/SquirrelLevel-darkmode.svg` (488 elements) is generated by
`tools/gen_kvantum.py`; `SquirrelLevel-darkmode.kvconfig` maps each widget to it.

**Qt Style Sheet** (`qt-style/SquirrelLevel-darkmode.qss`) — a lighter, engine-free
alternative that works in any Qt app: point qt5ct/qt6ct → *Appearance →
Style Sheets* at it, or launch an app with `-stylesheet SquirrelLevel-darkmode.qss`.
...
sudo plymouth-set-default-theme -R squirrellevel # set default (rebuilds initramfs)
```
...
- `SquirrelLevel-darkmoderc` — geometry: border thickness, title height, button size.
...
- **Title-bar height / button alignment** — tune `TitleHeight`,
  `ButtonHeight`, and `ButtonMarginTop` in `SquirrelLevel-darkmoderc`.
...
- **Border thickness** — `BorderLeft/Right/Bottom` in `SquirrelLevel-darkmoderc`.
